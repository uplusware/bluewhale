...
void bwgate_base::SetConfigFile(const char* config_file,
  ...
  while (getline(configfilein, strline)) {
    ...
    if (strncasecmp(strline.c_str(), "#", sizeof("#") - 1) != 0) {
      ...
      if (strcasecmp(strKey.c_str(), "CloseStderr") == 0) {
        ...
        strtrim(m_hostip);
      } else if (strcasecmp(strKey.c_str(), "InstanceMaxCocurrentConNum") ==
                 0) {
...
void bwgate_base::_load_permit_() {
  ...
  if (!permitfilein.is_open()) {
    fprintf(stderr, "%s is not exist. please creat it",
            m_permit_list_file.c_str());
    return;
  }
